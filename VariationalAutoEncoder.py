...
class VAE(tf.keras.Model):
    def __init__(self, latent_dim, inference_net=encoder2, generative_net=decoder2,
                 optimizer=tf.keras.optimizers.Adam(1e-4), kl_importance=1, keep_prob=0.8):
        ...
        self.r_loss = tf.keras.metrics.Mean("train_reconstruction_loss")
        self.kl_loss = tf.keras.metrics.Mean("kl_loss")
        self.total_loss = tf.keras.metrics.Mean("total_loss")

        self._metrics += [self.r_loss, self.kl_loss, self.total_loss]
    ...
    def calculate_reconstruction_loss(self, x, x_pred):
        return tf.norm(tf.reshape(x, [x.shape[0], -1]) - tf.reshape(x_pred, [x_pred.shape[0], -1]), axis=1)
    ...
    def compute_loss(self, x, training=False):
        ...
        rec_loss = self.calculate_reconstruction_loss(x, x_pred)
        self.r_loss(rec_loss)
